 ...
 *   Nikkei 225 CLOSED 64,141.12, -2,694 pts, -4.03%; intraday briefly
 *   -4,130.94 pts / -6.18% from Thursday's close; "Tokyo down more than
 *   5%"; index at a June low. (Prior close ≈66,835; 2694/66835 = 4.03% ✓;
 *   4130.94/66835 = 6.18% ✓; intraday low ≈62,704 — it fell ~4,130
 *   intraday and recovered to close -2,694.)
 *   ⚠️ CORRECTED 2026-07-22 — this episode originally published 63,674 /
 *   -3,161 pts / -4.73%. That was WRONG. Correct close is 64,141.12
 *   (-4.03%), confirmed by ts2.tech and 5paisa, and decisively by
 *   arithmetic off Tue 7/21's reopen: 66,231 / 1.0326 = 64,141 ✓
 *   (whereas 63,674 x 1.0326 = 65,750 ✗).
 ...
 *   64,141 close (7/17); the intermediate July points are an INTERPOLATED
 ...
 *   don't pile in at the top; ep69 B said 分批). Tonight it's 64,141 —
...
export const EPISODE_70: Episode = {
  ...
  actions: [
...

    {
      ...
      data: {
        tickers: [
          { symbol: 'Nikkei 225（7/17 收、盤中一度 -6.18%）', price: 64141.0, change: -4.03 },
        ...
        rsi: 28.0,
        beta: 1.4,
        priceHistory: [72353, 69174, 68200, 67400, 66835, 64500, 62704, 64141],
        ...
        sectors: [
          ...
          { label: 'Nikkei 225 7/17 -4.03% 收 64,141（-2,694 點、盤中一度 -4,130 點／-6.18%、回到六月低點）', value: 473, color: '#dd3333' },
        ],
        headline: '禮拜五 21:00 JST：AI 大回頭——Nikkei -4.03% 收 64,141（盤中一度 -6.18%／-4,130 點、六月低點）、Kioxia -16.1% Taiyo Yuden -11.9% SoftBank -9% 東京威力科創 -8.1%、台灣 -6.5% 台積電 -7.3%。矛盾在這：TSMC 財報破紀錄（營收 $40.2B、毛利 67.7%、營益率 60.3%、2026 營收成長上修 30%↑→40%↑）股價反崩——因同份報告 capex 拉到 $60-64B（+15%）＋美國 $100B＋海外廠稀釋毛利 2-4%：市場改問「錢誰出、股東分到什麼」/ 伊朗回來了：停火宣布結束、美軍連三天空襲、IRGC 打兩艘油輪＋科威特巴林基地、Hormuz 關閉、Brent $85.92（6/15 來最高）→ 油貴＋Warsh 鷹派、7/28-29 會議 21% 機率升息',
      },
    },
    ...
    { type: 'dialogue', character: 'chef', text: 'いらっしゃい、兩位一起到、稀客。這種天氣、先生啤酒、別跟我客氣。今天我一開店就在等你們——我老婆早上把報紙拍在桌上、說日經跌到 64,141。師傅你知道嗎、她第一次問我該不該買的時候、是 65,000。她從 65,000 問到 72,000、每次都沒買、今天跌回來、比她最早問的還低。她今天問我一句、我答不出來——「所以我猶豫了兩個月、其實是對的？」' },
    ...
    { type: 'dialogue', character: 'customer-a', text: '師傅、b 講得很重、但很對、我補一個角度。你太太這兩個月、其實一直在「等一個永遠不會來的訊號」——她在等「現在買絕對安全」的那一刻。那一刻不存在。所以她從 65,000 等到 72,000、又等回 64,141、看起來省了錢、其實她這兩個月什麼都沒學到、下次一樣不知道怎麼辦。如果她那時候真的照我們講的、分成幾份、在 65,000 放第一份——今天她會小賠、但她手上會有計畫、有剩下的錢、而且今天這個價、對她反而是機會。有計畫的小賠、比沒計畫的僥倖、值錢太多。' },
    ...
    { type: 'dialogue', character: 'customer-b', text: '師傅、鮎魚、太好了。而且今天是禮拜五、剛好。我爸做了三十年、留給我一句話、我每個禮拜五都會想起來：「週五收盤手上要乾淨、別把煩惱帶回家過週末。」今天收盤前、我把該處理的處理完了、沒留一個會讓我禮拜六早上醒來心跳加速的東西。師傅、跌 4.03% 的日子、我沒有多賺、但我可以好好過這個週末——這件事的價值、我年輕的時候完全不懂。' },
    ...
    {
      type: 'narration',
      text: '禮拜五晚上 10 點半、東京、還是很熱。今天的故事是「太早下結論」——三個人、三個版本、同一個病。市場端：Nikkei -4.03% 收 64,141（-2,694 點、盤中一度 -4,130 點／-6.18%、回到六月低點）、Kioxia -16.1%、Taiyo Yuden -11.9%、SoftBank -9%、東京威力科創 -8.1%、台灣 -6.5%、台積電 -7.3%。核心矛盾：台積電財報破紀錄（營收 $40.2B、毛利 67.7%、營益率 60.3%、2026 營收成長上修 30%↑→40%↑）、股價卻崩——因為同一份報告把 capex 拉到 $60-64B（+15%）、加美國 $100B 承諾、還自認海外廠稀釋毛利 2-4%。ep69「需求是真的」是對的、但只是上半句；下半句是「需求是真的、不等於你會賺到錢」（師傅:「生意好到要開二店、押金裝潢請人的錢從本店口袋出、二店還沒本店好賺——所以生意越好、口袋越空。我跟台積電煩惱一樣、只是零比較少」）。B 用 ep69 的工具量今天、答案是反的：六月是「被逼著砍」、今天是「想通了才賣」——「人被逼走、風頭過了會回來；人改變主意、要等他再被說服一次」（師傅:「客人沒來、是下雨、還是覺得我變難吃了」）。A 吃書：三禮拜前他在這張桌上「安靜送走」伊朗線、如今停火宣布結束、美軍連三天空襲、IRGC 打兩艘油輪＋科威特巴林基地、Hormuz 關閉、Brent $85.92（6/15 來最高）——他的錯不是猜錯方向、是「我宣布了一件事結束、而我沒有這個權力；我把『我希望它結束』包裝成『它結束了』」（師傅師父:「颱風走沒走、不是新聞說了算、是你把木板拆掉那天晚上還睡不睡得著」）。A 改規矩：以後只講「這條線很安靜」、不講「結束」；湯底不動、「留」多一格給「它只是在喘氣」。B 的 ep61 條件「Hormuz 真有船過」從沒觸發、反而倒轉、第三批從沒進場——但她拒絕居功：「我完全沒看出來會打起來、救我的不是腦袋、是三禮拜前寫下的那個條件——這不是先知、是懶惰的紀律」。A 收束：「她把條件寫死、我把結論講死；條件會保護你、結論會害你」（師傅:「三十年了、我每次出包、都是我先下結論那幾次」）。師傅老婆從 65,000 問到 72,000 沒買、今天 64,141 比最初還低、問「所以我的猶豫是對的？」——答案：猶豫 ≠ 紀律、這次結果一樣但骨子不同、僵住不會重複、計畫才會（A:「有計畫的小賠、比沒計畫的僥倖值錢太多」）。油鏈：Hormuz 關→油貴→運費／炸油／瓦斯全漲→政府更不敢降息→錢貴→沒人願意高價買「要等很多年才賺錢」的 AI 股（師傅:「我巷子裡付的炸油錢、跟他們螢幕上的數字、是同一條線的兩頭」；7/28-29 會議 21% 機率升息、半年前沒人敢想）。夏夜收尾：鱧魚湯引、鮎魚鹽烤、冬瓜、冷酒——B 她爸「週五收盤手上要乾淨、別把煩惱帶回家過週末」；A 加一句「手上乾淨不只是倉位、還有腦袋、拆一個結論比拆一個倉位難多了」。師傅收在三十年的那句：「只要你敢說『我懂了』、它明天就給你看一個你沒見過的東西——所以我只敢說『我今天煮得還可以』。」',
      duration: 6000,
    },
  ],
};
